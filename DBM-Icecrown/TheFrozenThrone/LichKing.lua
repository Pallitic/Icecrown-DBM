...
local ttsDefileCountdown = mod:NewSoundFile("Interface\\AddOns\\DBM-Core\\sounds\\defilecd.mp3", "TTS Defile Countdown", false)
local ttsDefileOnYou = mod:NewSoundFile("Interface\\AddOns\\DBM-Core\\sounds\\defileonyou.mp3", "TTS Defile on you callout", true)
local ttsSoulreaperSoon = mod:NewSoundFile("Interface\\AddOns\\DBM-Core\\sounds\\reapercd.mp3", "TTS Soulreaper Countdown", mod:IsTank() or mod:IsHealer())
local ttsRaging= mod:NewSoundFile("Interface\\AddOns\\DBM-Core\\sounds\\ragingcd.mp3", "TTS Raging countdown", false)
local ttsNecro= mod:NewSoundFile("interface\\AddOns\\DBM-Core\\sounds\\necro.mp3", "TTS Necrotic on you", true)
local ttsNecrotic = mod:NewSoundFile("Interface\\AddOns\\DBM-Core\\sounds\\necroticcd.mp3", "TTS Necrotic countdown", false)
local ttsInfestSoon = mod:NewSoundFile("Interface\\AddOns\\DBM-Core\\sounds\\infestcd.mp3", "TTS Infest countdown", mod:IsHealer())
local ttsChamber = mod:NewSoundFile("Interface\\AddOns\\DBM-Core\\sounds\\chambercd.mp3", "TTS Harvest soul countdown", false)
local ttsEnrage =  mod:NewSoundFile("Interface\\AddOns\\DBM-Core\\sounds\\enragecd.mp3", "TTS Enrage countdown", mod:IsTank() or mod:CanRemoveEnrage())
local ttsValk = mod:NewSoundFile("Interface\\AddOns\\DBM-Core\\sounds\\valkcd.mp3", "TTS Valk grabs countdown", false)
local ttsNearST = mod:NewSoundFile("Interface\\AddOns\\DBM-Core\\sounds\\shadowtrap.mp3", "TTS Near shadowtrap calls", true)
local ttsInfest = mod:NewSoundFile("Interface\\Addons\\DBM-Core\\sounds\\infest.mp3", "TTS infest cast", true)
local ttsReaper = mod:NewSoundFile("Interface\\Addons\\DBM-Core\\sounds\\soulreaper.mp3", "TTS Reaper cast", mod:IsTank() or mod:IsHealer())
...
function mod:SPELL_CAST_START(args)
	...
	elseif args:IsSpellID(72762) then -- Defile
		...
		timerDefileCD:Start()
		ttsDefileCountdown:Cancel()
		ttsDefileCountdown:Schedule(27)
	...
	elseif args:IsSpellID(73650) then -- Restore Soul (Heroic)
		...
		warnDefileSoon:Schedule(37)
        ttsDefileCountdown:Schedule(36)
		timerVileSpirit:Start(48)
		timerSoulreaperCD:Start(45)
		ttsSoulreaperSoon:Schedule(38)
...
function mod:SPELL_CAST_SUCCESS(args)
	...
	elseif args:IsSpellID(69200) then -- Raging Spirit
		...
		if self.vb.phase == 1 then
			...
			ttsRaging:Schedule(17)
		else
			timerRagingSpiritCD:Start(17)
			ttsRaging:Schedule(12)
		end
...
function mod:NextPhase()
	...
	elseif self.vb.phase == 2 then
		...
		ttsSoulreaperSoon:Cancel()
		ttsSoulreaperSoon:Schedule(33)
		timerDefileCD:Start(38)
		ttsDefileCountdown:Schedule(32)
		timerInfestCD:Start(14)
		ttsInfestSoon:Cancel()
		ttsInfestSoon:Schedule(9)
