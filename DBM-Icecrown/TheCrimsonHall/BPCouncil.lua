...
local ttsTargetSwitch = mod:NewSoundFile("Interface\\AddOns\\DBM-Core\\sounds\\icc\\bpc\\TargetSwitch_Soon.wav", "TTS Target switch Countdown", true)
local ttsVortexCast = mod:NewSoundFile("Interface\\AddOns\\DBM-Core\\sounds\\icc\\bpc\\VortexCast.wav", "TTS Shock vortex cast", true)
local ttsFireOrb = mod:NewSoundFile("Interface\\AddOns\\DBM-Core\\sounds\\icc\\bpc\\fireorb.mp3", "TTS Flame orb on you", true)
local ttsStacks = mod:NewSoundFile("Interface\\AddOns\\DBM-Core\\sounds\\icc\\bpc\\stacks.wav", "TTS Too many stacks", true)
local ttsVortex = mod:NewSoundFile("Interface\\AddOns\\DBM-Core\\sounds\\icc\\bpc\\SpreadOut.wav", "TTS Empowered Vortex cast", true)
...
local function SPMF_do_metronom(self, elapsed) -- play sound every second
	...
	if self.time >= 1 then
		...
		PlaySoundFile("Interface\\AddOns\\DBM-Core\\sounds\\icc\\bpc\\tick.mp3", "Master")
...
function mod:OnCombatStart(delay)
	...
	ttsTargetSwitch:Schedule(42-delay)
...
function mod:SPELL_AURA_APPLIED(args)
	...
	elseif args:IsSpellID(72999) then	--Shadow Prison (hard mode)
		...
		if args:IsPlayer() then
			...
			if (args.amount or 1) >= 8 then	--Placeholder right now, might use a different value
