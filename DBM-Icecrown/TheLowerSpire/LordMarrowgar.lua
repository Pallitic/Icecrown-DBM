...
local timerWhirlwindCD		= mod:NewCDTimer(70, 69076)
local timerWhirlwind		= mod:NewBuffActiveTimer(37, 69076)
...
local ttsBoneCD = mod:NewSoundFile("Interface\\AddOns\\DBM-Core\\sounds\\icc\\marrow\\bonestormcd.mp3", "TTS Bone storm Countdown", true)
...
function mod:OnCombatStart(delay)
	preWarnWhirlwind:Schedule(43-delay)
	timerWhirlwindCD:Start(48-delay)
	ttsBoneCD:Schedule(43-delay)
...
function mod:SPELL_AURA_APPLIED(args)
	if args:IsSpellID(69076) then		-- Bone Storm (Whirlwind)
		...
		ttsBoneCD:Schedule(65)
		preWarnWhirlwind:Schedule(65)
		timerWhirlwind:Show()
		soundWhirlwind:Play()
	end
end
...
function mod:SPELL_CAST_START(args)
	if args:IsSpellID(69057, 70826, 72088, 72089) then		-- Bone Spike Graveyard
		...
		PlaySoundFile("Interface\\Addons\\DBM-Core\\sounds\\icc\\marrow\\castingbonespikes.mp3")
	end
end
