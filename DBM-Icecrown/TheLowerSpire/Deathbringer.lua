...
local ttsBeastCD = mod:NewSoundFile("Interface\\AddOns\\DBM-Core\\sounds\\icc\\dbs\\beastscd.mp3", "TTS Beasts countdown", false)
local ttsVanishOnMark = mod:NewSoundFile("Interface\\AddOns\\DBM-Core\\sounds\\icc\\dbs\\vanish.mp3", "TTS vanish callout on mark if rogue", select(2, UnitClass("player")) == "ROGUE")
local ttsShadowmeldOnMark = mod:NewSoundFile("Interface\\AddOns\\DBM-Core\\sounds\\icc\\dbs\\onYou.mp3", "TTS 'on you' callout if you can shadowmeld mark", select(2, UnitRace("player")) == "NightElf")
local ttsBeasts = mod:NewSoundFile("Interface\\AddOns\\DBM-Core\\sounds\\icc\\dbs\\beasts.mp3", "TTS Beasts summon", true)																																											 
local ttsTanks = mod:NewSoundFile("Interface\\AddOns\\DBM-Core\\sounds\\uld\\alg\\SwitchTanks.wav", "TTS Switch tanks", mod:IsTank())
...
function mod:OnCombatStart(delay)
	...
	ttsBeastCD:Schedule(35)
	timerBloodNova:Start(-delay)
	timerRuneofBlood:Start(15-delay)
