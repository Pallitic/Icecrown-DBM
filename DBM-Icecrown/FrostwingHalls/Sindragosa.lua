...
local ttsBeacon = mod:NewSoundFile("Interface\\AddOns\\DBM-Core\\sounds\\beaconOnYou.mp3", "TTS Beacon on you", false)
local ttsHighChilledToBone = mod:NewSoundFile("Interface\\AddOns\\DBM-Core\\sounds\\highChilledToBone.mp3", "TTS High Chilled stacks", false)
local ttsUnchained = mod:NewSoundFile("Interface\\AddOns\\DBM-Core\\sounds\\unchained.mp3", "TTS Unchained callout", false)
local ttsFrostBreathCountdown = mod:NewSoundFile("Interface\\AddOns\\DBM-Core\\sounds\\breathcd.mp3", "TTS frost breath countdown", mod:IsTank())
local ttsFrostBreathCountdownOffset = 6.8
local ttsBlisteringCountdown = mod:NewSoundFile("Interface\\AddOns\\DBM-Core\\sounds\\blisteringcd.mp3", "TTS blistering cold countdown", true)
local ttsBlisteringCountdownOffset = 6
...
function mod:CHAT_MSG_MONSTER_YELL(msg)
	...
	elseif (msg == L.YellPhase2 or msg:find(L.YellPhase2)) or (msg == L.YellPhase2Dem or msg:find(L.YellPhase2Dem)) then
		...
		ttsBlisteringCountdown:Schedule(35-ttsBlisteringCountdownOffset)
	end
end